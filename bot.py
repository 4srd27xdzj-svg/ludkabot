...
SETTING_STATS_UTC_OFFSET_MINUTES = "stats_utc_offset_minutes"
SETTING_OWNER_BANK_USER_ID = "owner_bank_user_id"

DEFAULT_STATS_UTC_OFFSET_MINUTES = 0
STATS_RANGE_INPUT_FORMATS = (
    ("%d.%m.%Y %H:%M", False),
    ("%Y-%m-%d %H:%M", False),
    ("%d.%m.%Y", True),
    ("%Y-%m-%d", True),
...
TEMPLATE_DEFAULTS = {
    ...
    "mystats_target_empty": "У игрока <b>{username}</b> пока нет прокрутов.",
    "user_not_found": "Игрок <b>{username}</b> не найден в статистике.",
    "fullstats": (
        "<b>Полная статистика за всё время</b>\n\n"
        "Всего прокрутов: <b>{total_spins}</b>\n"
        "777: <b>{jackpots}</b>\n"
        "Открыто кейсов: <b>{opened_cases}</b>\n"
        "NFT: <b>{nft_wins}</b>\n"
    ...
    "fullstats_row": (
        "<b>{place}. {username}</b> — спины: <b>{total_spins}</b>, "
        "777: <b>{jackpots}</b>, NFT: <b>{nft_wins}</b>"
    ),
    "fullstats_empty": "Пока нет игроков с прокрутами.",
    "owner_payout": (
        "<b>Новый выигрыш</b>\n\n"
        "Игрок: <b>{username}</b>\n"
        "Тип: <b>{result_type}</b>\n"
    ...
    "stats_range_prompt": (
        "<b>Статистика за свой период</b>\n\n"
        "Отправьте следующим сообщением начало и конец через символ <code>|</code>:\n"
        "<code>23.08.2026 10:00 | 23.08.2026 18:00</code>\n\n"
        "Часовой пояс: <b>{stats_timezone}</b>\n"
        "Изменить: <code>/statsrange timezone +03:00</code>"
    ),
    "stats_range_invalid": (
        "Не удалось получить статистику: <b>{error}</b>\n\n"
        "Пример: <code>23.08.2026 10:00 | 23.08.2026 18:00</code>"
    ),
    "stats_timezone_saved": (
        "Часовой пояс статистики установлен: <b>{stats_timezone}</b>."
    ),
    "game_settings": (
        "<b>Настройки игры</b>\n\n"
        "Кейсов: <b>{case_count}</b>\n"
    ...
    "reset_user_stats_usage": (
        "Укажите username игрока:\n"
        "<code>/resetuserstats @username</code>"
    ),
    "reset_user_stats_private_only": "Эта команда работает только в личных сообщениях бота.",
    "reset_user_stats_done": "Статистика игрока <b>{username}</b> полностью обнулена.",
    ...
    "case_send_failed": (
        "777 зафиксирован, но Telegram не принял поле. "
        "Администратор уже получил ошибку в логах."
    ),
    "payout_done_alert": "Выдача отмечена завершенной.",
    "paid_already_alert": "Уже завершено.",
    "chat_id": "chat_id: {chat_id}\nuser_id: {user_id}",
    "giftbank": (
        "<b>Банк лимитированных NFT</b>\n"
        "Аккаунт-банк: <code>{owner_bank_user_id}</code>\n\n"
        "{owner_gifts}\n\n"
        "{blocked_gifts}\n\n"
        "Команды:\n"
        "<code>/ownerbank USER_ID</code>\n"
        "<code>/ownerbank me</code>\n"
        "<code>/giftbank</code>\n"
        "<code>/gift block https://t.me/nft/name</code>\n"
        "<code>/gift unblock https://t.me/nft/name</code>"
    ),
    "giftbank_empty": "На аккаунте-банке не найдено доступных лимитированных NFT.",
    "giftbank_fetch_error": "Не удалось получить NFT аккаунта-банка: <code>{error}</code>",
    "giftbank_blocked_header": "<b>Заблокировано:</b>\n{blocked_list}",
    "owner_bank_empty": (
        "Аккаунт-банк не настроен. Используйте <code>/ownerbank USER_ID</code> "
        "или <code>/ownerbank me</code>."
    ),
    "owner_bank_saved": "Аккаунт-банк NFT установлен: <code>{owner_bank_user_id}</code>.",
    "owner_bank_cleared": "Аккаунт-банк NFT отключен.",
    "owner_bank_status": (
        "<b>Аккаунт-банк NFT</b>\n\n"
        "Текущий ID: <code>{owner_bank_user_id}</code>\n"
        "В банк входят только лимитированные подарки NFT."
    ),
    "texts_panel": (
    ...
    "help": (
        ...
        "/fullstats - полная статистика и все игроки"
    ),
}
...
class Database:
    ...
    def get_user_row(self, user_id: int) -> sqlite3.Row | None:
        return self.connection.execute(
            "SELECT * FROM users WHERE user_id = ?",
            (user_id,),
        ).fetchone()

    def get_all_stat_rows(self, chat_id: int) -> list[sqlite3.Row]:
        return list(
            self.connection.execute(
                """
                SELECT s.*, u.username, u.first_name, u.last_name
    ...
    def reset_user_stats(self, user_id: int) -> None:
        self.connection.execute("DELETE FROM slot_stats WHERE user_id = ?", (user_id,))
        self.connection.execute("DELETE FROM game_events WHERE user_id = ?", (user_id,))
        self.connection.execute(
            "UPDATE case_rounds SET status = 'cancelled' "
...
def get_owner_bank_user_id(db: Database) -> int | None:
    try:
        user_id = int(db.get_setting(SETTING_OWNER_BANK_USER_ID, 0))
...
def parse_owned_gift(raw: dict[str, object]) -> dict[str, object] | None:
    ...
    # Regular gifts are not limited NFTs and never belong to the game bank.
    return None
...
async def fetch_owner_gifts(
    ...
    if gift_type not in {None, "nft"}:
        return []

    owner_bank_user_id = get_owner_bank_user_id(db)
    if owner_bank_user_id is None:
        return []
    ...
    while True:
        ...
        for raw_gift in raw_gifts:
            ...
            if parsed is None or parsed["type"] != "nft":
                continue
            key = gift_key(parsed)
            block_keys = {
                key,
            ...
            if key in seen_keys:
                continue
            if any(value and db.is_owner_gift_blocked(value) for value in block_keys):
                continue
            seen_keys.add(key)
            gifts.append(parsed)
...
async def choose_owner_gift(
    ...
    if gift_type not in {None, "nft"}:
        return None
    try:
        gifts = await fetch_owner_gifts(config, db, "nft")
    except Exception as error:
        logging.warning("Cannot fetch owner gifts: %s", error)
        gifts = []

    return random.choice(gifts) if gifts else None
...
def get_stats_utc_offset_minutes(db: Database) -> int:
    try:
        offset = int(db.get_setting(
            SETTING_STATS_UTC_OFFSET_MINUTES,
            DEFAULT_STATS_UTC_OFFSET_MINUTES,
        ))
    except (TypeError, ValueError):
        return DEFAULT_STATS_UTC_OFFSET_MINUTES
    return offset if -12 * 60 <= offset <= 14 * 60 else DEFAULT_STATS_UTC_OFFSET_MINUTES


def format_stats_timezone(offset_minutes: int) -> str:
...
def parse_stats_timezone(value: str) -> int:
    normalized = value.strip().upper().replace("UTC", "").replace("−", "-")
    match = re.fullmatch(r"([+-]?)(\d{1,2})(?::?(\d{2}))?", normalized)
    if not match:
        raise ValueError("часовой пояс должен выглядеть как +03:00")

    sign = -1 if match.group(1) == "-" else 1
    hours = int(match.group(2))
    minutes = int(match.group(3) or 0)
    if minutes >= 60:
        raise ValueError("в часовом поясе указаны неверные минуты")

    offset = sign * (hours * 60 + minutes)
    if not -12 * 60 <= offset <= 14 * 60:
        raise ValueError("часовой пояс должен быть от UTC-12:00 до UTC+14:00")
    return offset


def parse_stats_range_datetime(
    value: str,
    offset_minutes: int,
    is_end: bool,
) -> datetime:
    ...
    date_only = False
    for date_format, format_is_date_only in STATS_RANGE_INPUT_FORMATS:
        try:
            parsed = datetime.strptime(normalized, date_format)
            date_only = format_is_date_only
            break
        except ValueError:
            continue
    if parsed is None:
        raise ValueError(f"неверная дата: {value.strip()}")
    if date_only and is_end:
        parsed = parsed.replace(hour=23, minute=59, second=59)

    local_timezone = timezone(timedelta(minutes=offset_minutes))
    return parsed.replace(tzinfo=local_timezone).astimezone(timezone.utc).replace(tzinfo=None)


def parse_stats_range(value: str, offset_minutes: int) -> tuple[datetime, datetime]:
    parts = [part.strip() for part in value.split("|")]
    if len(parts) != 2 or not all(parts):
        raise ValueError("укажите начало и конец через символ |")

    since = parse_stats_range_datetime(parts[0], offset_minutes, is_end=False)
    until = parse_stats_range_datetime(parts[1], offset_minutes, is_end=True)
    if until < since:
        raise ValueError("конец периода должен быть позже начала")
    return since, until


def format_stats_range_boundary(value: datetime, offset_minutes: int) -> str:
    local_timezone = timezone(timedelta(minutes=offset_minutes))
    local_value = value.replace(tzinfo=timezone.utc).astimezone(local_timezone)
    ...
    for position in range(1, case_count + 1):
        ...
        current_row.append(
            case_inline_button(
                ...
                get_case_icon_id(db, "closed") if include_icons else "",
                (
                    (
...
def plain_case_keyboard(
    db: Database,
    round_id: int,
    case_count: int,
) -> InlineKeyboardMarkup:
    configured_label = get_case_closed_label(db)
    label = (
        configured_label
        if configured_label and configured_label != BUTTON_ICON_ONLY_TEXT
        else "□"
    )
    side = max(1, round(case_count**0.5))
    rows: list[list[InlineKeyboardButton]] = []
    for start in range(1, case_count + 1, side):
        rows.append(
            [
                InlineKeyboardButton(
                    label,
                    callback_data=f"case:{round_id}:{position}",
                )
                for position in range(start, min(start + side, case_count + 1))
            ]
        )
    return InlineKeyboardMarkup(rows)
    ...
    for position in range(1, case_count + 1):
        ...
        current_row.append(
            case_inline_button(
                text,
                f"case:{round_id}:{position}",
                result_button_icon_id(db, result_type, cell) if include_icons else "",
                (
                    (
                        get_case_button_style(db, "selected")
                        if is_selected
...
def owner_panel_keyboard() -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(
        [
            ...
            [InlineKeyboardButton("NFT-банк", callback_data="panel:ownerbank")],
        ]
    )


def owner_bank_keyboard() -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(
        [
            [
                InlineKeyboardButton(
                    "Сделать меня банком",
                    callback_data="panel:ownerbank:me",
                ),
                InlineKeyboardButton(
                    "Отключить банк",
                    callback_data="panel:ownerbank:clear",
                ),
            ],
            [InlineKeyboardButton("Показать NFT", callback_data="panel:gifts")],
        ]
    )
...
def stats_period_keyboard() -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(
        [
            ...
            [InlineKeyboardButton("Свой период", callback_data="panel:stats:range")],
        ]
    )
...
def build_template_values(
    ...
    period_title_override: str | None = None,
) -> dict[str, object]:
    case_layout = get_case_layout(db)
    stats_offset = get_stats_utc_offset_minutes(db)
    owner_bank_user_id = get_owner_bank_user_id(db)
    values: dict[str, object] = {
        "username": display_name_from_user(user) if user else "",
        "owner_bank_user_id": owner_bank_user_id or "не настроен",
        "stats_timezone": format_stats_timezone(stats_offset),
        "range_start": "",
        "range_end": "",
        "case_count": get_case_count(db),
    ...
    if chat_id is not None:
        is_custom_range = (
            stats_since is not None
            or stats_until is not None
            or period_title_override is not None
        )
        if is_custom_range:
            period_title = period_title_override or "за выбранный период"
            totals = db.get_event_totals(chat_id, stats_since, stats_until)
            top10 = format_top10(
                db,
                chat_id,
                period_key="range",
            ...
            values["range_start"] = (
                format_stats_range_boundary(stats_since, stats_offset)
                if stats_since is not None
                else ""
            )
            values["range_end"] = (
                format_stats_range_boundary(stats_until, stats_offset)
                if stats_until is not None
                else ""
            )
        else:
...
def format_top10(
    ...
    if since is not None or until is not None:
        rows = db.get_event_top_rows(chat_id, since, until)
    else:
        _, delta = STATS_PERIODS.get(period_key, STATS_PERIODS["all"])
        period_since = datetime.utcnow() - delta if delta else None
        rows = (
            db.get_event_top_rows(chat_id, period_since)
            if delta
            else db.get_top_rows(chat_id)
        )
    if not rows:
...
def format_all_stat_rows(db: Database, chat_id: int) -> str:
    rows = db.get_all_stat_rows(chat_id)
    if not rows:
        return template_text(db, "fullstats_empty")
    ...
    return "\n".join(
        template_text(
            ...
            extra={
                ...
                "total_spins": row["total_spins"],
                "jackpots": row["jackpots"],
                "opened_cases": row["opened_cases"],
                "nft_wins": row["nft_wins"],
                "gift_wins": row["gift_wins"],
                "empty_wins": row["empty_wins"],
                "gross_stars": format_stars(float(row["gross_stars"])),
                "net_stars": format_stars(float(row["net_stars"])),
            },
        )
        for index, row in enumerate(rows, start=1)
    )
...
def full_stats_text(db: Database, chat_id: int) -> str:
    values = build_template_values(db, chat_id=chat_id, period_key="all")
    values["all_users"] = format_all_stat_rows(db, chat_id)
    return render_template_with_raw(db, "fullstats", values, {"all_users"})
    ...
    for line in text.splitlines(keepends=True):
        ...
        if len(line) > max_length:
            if current:
                ...
                current = ""
            chunks.extend(
                line[index : index + max_length].rstrip()
...
async def handle_owner_panel(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if action == "game":
        ...
        reply_markup = None
    elif action == "ownerbank":
        bank_action = parts[2] if len(parts) >= 3 else "show"
        if bank_action == "me":
            db.set_setting(SETTING_OWNER_BANK_USER_ID, query.from_user.id)
            text = template_text(db, "owner_bank_saved", query.from_user)
        elif bank_action == "clear":
            db.set_setting(SETTING_OWNER_BANK_USER_ID, 0)
            text = template_text(db, "owner_bank_cleared", query.from_user)
        else:
            template_key = (
                "owner_bank_status"
                if get_owner_bank_user_id(db) is not None
                else "owner_bank_empty"
            )
            text = template_text(db, template_key, query.from_user)
        reply_markup = owner_bank_keyboard()
    elif action == "gifts":
        ...
        if len(parts) >= 3:
            ...
            reply_markup = templates_keyboard()
    elif action == "stats" and len(parts) >= 3 and parts[2] == "range":
        context.user_data["awaiting_stats_range"] = True
        text = template_text(db, "stats_range_prompt", query.from_user)
        reply_markup = stats_period_keyboard()
    elif action == "stats" and len(parts) >= 3:
        context.user_data.pop("awaiting_stats_range", None)
...
async def manage_gifts(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    try:
        if action == "add":
            raise ValueError(
                "Ручные подарки отключены. NFT загружаются только с аккаунта /ownerbank."
            )
...
async def manage_owner_bank(
    ...
    if not is_owner(config, update.effective_user.id):
        logging.warning(
            "Ignored /ownerbank from non-owner user_id=%s",
    ...
    if not context.args:
        ...
        await send_html(update.message, template_text(db, template_key))
        return

    raw_target = context.args[0].strip()
    if raw_target.lower() in {"clear", "off", "none", "удалить", "выключить"}:
        db.set_setting(SETTING_OWNER_BANK_USER_ID, 0)
        await send_html(update.message, template_text(db, "owner_bank_cleared"))
        return

    try:
        if raw_target.lower() in {"me", "я"}:
            bank_user_id = update.effective_user.id
        elif raw_target.startswith("@"):
            found_user_id = db.find_user_id_by_username(raw_target)
            if found_user_id is None:
                raise ValueError(
                    "Этот @username пока не известен боту. Укажите числовой user ID."
                )
            bank_user_id = found_user_id
        else:
            bank_user_id = int(raw_target.lstrip("="))

        if bank_user_id not in config.owner_user_ids:
            raise ValueError(
                "Аккаунт-банк должен входить в OWNER_USER_IDS."
            )
    except ValueError as error:
        await send_html(
            update.message,
            template_text(db, "validation_error", extra={"error": error}),
        )
        return

    db.set_setting(SETTING_OWNER_BANK_USER_ID, bank_user_id)
    await send_html(update.message, template_text(db, "owner_bank_saved"))
...
async def gifts_text(config: BotConfig, db: Database) -> str:
    ...
    owner_bank_user_id = get_owner_bank_user_id(db)
    if owner_bank_user_id is None:
        owner_gifts: list[dict[str, object]] = []
        owner_lines.append(template_text(db, "owner_bank_empty"))
    else:
        try:
            owner_gifts = await fetch_owner_gifts(config, db, "nft")
        except Exception as error:
            owner_gifts = []
            owner_lines.append(
                render_template(
...
def templates_text(db: Database) -> str:
    ...
    placeholders = (
        ...
        "<code>{period}</code>, <code>{period_title}</code>, <code>{range_start}</code>, "
        "<code>{range_end}</code>, <code>{stats_timezone}</code>,\n"
        "<code>{owner_bank_user_id}</code>, <code>{result_type}</code>, "
...
async def show_stats(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    raw_range = " ".join(context.args).strip()
    if raw_range:
        await send_stats_range_from_text(update, context, raw_range)
        return
    ...
    if update.effective_chat.type == "private":
        ...
        await send_html(
            update.message,
            template_text(db, "stats_choose_period", update.effective_user),
...
def stats_range_text(
    db: Database,
    chat_id: int,
    since: datetime,
    until: datetime,
) -> str:
    offset_minutes = get_stats_utc_offset_minutes(db)
    range_start = format_stats_range_boundary(since, offset_minutes)
    range_end = format_stats_range_boundary(until, offset_minutes)
    period_title = (
        f"с {range_start} по {range_end} "
        f"({format_stats_timezone(offset_minutes)})"
    )
    values = build_template_values(
        ...
        stats_since=since,
        stats_until=until,
        period_title_override=period_title,
    )
    return render_template_with_raw(db, "stats", values, {"top5", "top10"})


def stats_chat_id_for_update(update: Update, config: BotConfig) -> int | None:
    if update.effective_chat and is_allowed_chat(config, update.effective_chat.id):
        return update.effective_chat.id
    return next(iter(config.allowed_chat_ids), None)


async def send_stats_range_from_text(
    update: Update,
    context: ContextTypes.DEFAULT_TYPE,
    raw_range: str,
) -> bool:
    ...
    db = db_from_context(context)
    config = config_from_context(context)
    chat_id = stats_chat_id_for_update(update, config)
    if chat_id is None:
        await send_html(update.message, template_text(db, "allowed_chats_empty"))
        return False

    try:
        since, until = parse_stats_range(
            raw_range,
            get_stats_utc_offset_minutes(db),
        )
    except ValueError as error:
        await send_html(
            update.message,
    ...
    await send_html(update.message, stats_range_text(db, chat_id, since, until))
    return True


async def show_stats_range(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    if not update.effective_user or not update.message:
        return
    ...
    if not raw_value:
        ...
        await send_html(
            update.message,
            template_text(db, "stats_range_prompt", update.effective_user),
            stats_period_keyboard(),
        )
        return

    timezone_parts = raw_value.split(maxsplit=1)
    if timezone_parts[0].lower() in {"timezone", "tz"}:
        if len(timezone_parts) != 2:
            await send_html(
                ...
                template_text(
                    ...
                    "stats_range_invalid",
                    update.effective_user,
                    extra={"error": "укажите часовой пояс, например +03:00"},
                ),
            )
            return
        try:
            offset_minutes = parse_stats_timezone(timezone_parts[1])
        except ValueError as error:
            await send_html(
                update.message,
                template_text(
            ...
            return
        db.set_setting(SETTING_STATS_UTC_OFFSET_MINUTES, offset_minutes)
        context.user_data.pop("awaiting_stats_range", None)
        await send_html(
            update.message,
            template_text(db, "stats_timezone_saved", update.effective_user),
        )
        return

    if await send_stats_range_from_text(update, context, raw_value):
        context.user_data.pop("awaiting_stats_range", None)


async def handle_stats_range_input(
    update: Update,
    context: ContextTypes.DEFAULT_TYPE,
) -> None:
    ...
    is_waiting = bool(context.user_data.get("awaiting_stats_range"))
    is_private_range = (
        update.effective_chat is not None
        and update.effective_chat.type == "private"
        and "|" in update.message.text
    )
    if not is_waiting and not is_private_range:
        return

    if await send_stats_range_from_text(update, context, update.message.text):
        context.user_data.pop("awaiting_stats_range", None)
...
def public_stats_chat_id(update: Update, config: BotConfig) -> int | None:
    if update.effective_chat is None:
        return None
    if update.effective_chat.type == "private":
        return next(iter(config.allowed_chat_ids), None)
    if is_allowed_chat(config, update.effective_chat.id):
        return update.effective_chat.id
    return None
...
async def show_my_stats(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    chat_id = public_stats_chat_id(update, config)
    if chat_id is None:
        if update.effective_chat.type != "private":
            return
        await send_html(update.message, template_text(db, "allowed_chats_empty"))
    ...
    target_row = db.get_user_row(target_user_id)
    requested_target = bool(context.args)
    if requested_target:
        raw_target = context.args[0].strip()
        if raw_target.lstrip("=").isdigit():
            target_user_id = int(raw_target.lstrip("="))
        else:
            found_user_id = db.find_user_id_by_username(raw_target)
            if found_user_id is None:
                username = raw_target if raw_target.startswith("@") else f"@{raw_target}"
                await send_html(
                    update.message,
                    template_text(
                        db,
    ...
    if target_row is None:
        await send_html(
            ...
            template_text(
                ...
                "user_not_found",
                update.effective_user,
                extra={"username": context.args[0] if context.args else target_user_id},
            ),
        )
        return

    target_name = display_name_from_row(target_row)
    stats = db.get_user_stats(chat_id, target_user_id)
    if stats is None:
        template_key = "mystats_target_empty" if requested_target else "mystats_empty"
        await send_html(
            update.message,
            template_text(
                db,
...
async def show_full_stats(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    await send_long_html(update.message, full_stats_text(db, chat_id))
...
async def reset_user_stats(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if update.effective_chat.type != "private":
        await send_html(
            update.message,
            template_text(db, "reset_user_stats_private_only", update.effective_user),
        )
        return
    ...
    if not context.args:
        await send_html(
            update.message,
            template_text(db, "reset_user_stats_usage", update.effective_user),
        )
        return

    raw_username = context.args[0].strip()
    target_user_id = db.find_user_id_by_username(raw_username)
    ...
    target_row = db.get_user_row(target_user_id)
    target_name = display_name_from_row(target_row) if target_row is not None else raw_username
    db.reset_user_stats(target_user_id)
    await send_html(
        update.message,
        template_text(
            db,
            "reset_user_stats_done",
            update.effective_user,
            extra={"username": target_name},
        ),
    )
...
async def auto_stats_loop(application: Application) -> None:
    ...
    while True:
        ...
        for chat_id in config.allowed_chat_ids:
            await send_stats_to_chat(application, chat_id, "all")
...
async def react_to_message(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    logging.info(
        "Jackpot detected chat_id=%s user_id=%s message_id=%s value=%s",
    ...
    try:
        ...
        logging.error(
            "Cannot create jackpot field chat_id=%s user_id=%s: %s",
            update.effective_chat.id,
            update.effective_user.id,
            error,
        ...
        try:
            await update.message.reply_text(
                strip_html_tags(template_text(db, "case_send_failed", update.effective_user))
            )
...
async def send_case_challenge(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    attempts = (
        (
            ...
            text,
            ParseMode.HTML,
            case_keyboard(db, round_id, case_count),
        ),
        (
            "icons_without_styles",
            text,
            ParseMode.HTML,
            case_keyboard(db, round_id, case_count, include_styles=False),
        ),
        (
            "icons_with_plain_text",
        ...
        (
            ...
            strip_html_tags(text),
            None,
            plain_case_keyboard(db, round_id, case_count),
        ),
    )
    sent_message = None
    for attempt_index, (attempt_name, attempt_text, parse_mode, reply_markup) in enumerate(
        attempts
    ):
        try:
            ...
            logging.warning(
                "Case keyboard attempt %s rejected for round_id=%s: %s",
                attempt_name,
                round_id,
                error,
            )
    ...
    if sent_message is None:
        raise RuntimeError("Telegram did not return a message for the jackpot field.")
...
async def handle_case_choice(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    for attempt_index, (attempt_name, reply_markup) in enumerate(reveal_attempts):
        try:
            await query.edit_message_reply_markup(reply_markup=reply_markup)
            break
        except TelegramError as error:
            logging.warning(
                "Revealed case keyboard attempt %s rejected for round_id=%s: %s",
                attempt_name,
                round_id,
                error,
            )
            if attempt_index == len(reveal_attempts) - 1:
                break
...
async def log_application_error(
    ...
    if error is None:
        return
    logging.error(
        "Unhandled Telegram update error update=%r: %s",
        update,
        error,
        exc_info=(type(error), error, error.__traceback__),
    )
...
def main() -> None:
    ...
    application.add_handler(CommandHandler("ownerbank", manage_owner_bank))
    application.add_handler(CommandHandler("bankowner", manage_owner_bank))
    application.add_handler(CommandHandler("settext", set_text))
    application.add_handler(CommandHandler("texts", set_text))
    application.add_handler(CommandHandler("stats", show_stats))
    application.add_handler(CommandHandler("statsrange", show_stats_range))
    ...
    application.add_handler(
        MessageHandler(filters.TEXT & ~filters.COMMAND, handle_stats_range_input)
    )
