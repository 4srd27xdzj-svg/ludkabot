...
DEFAULT_CASE_NFT_LABEL = "NFT"
DEFAULT_CASE_STARS15_LABEL = "15⭐"
DEFAULT_CASE_STARS25_LABEL = "25⭐"
...
def make_star_case_cell(position: int) -> dict[str, object]:
...
async def build_case_board(
    ...
    board = [make_star_case_cell(position) for position in range(1, case_count + 1)]
...
def revealed_case_keyboard(
    ...
    for position in range(1, case_count + 1):
        ...
        if selected_position == position:
            text = selected_prize_label(result_type, cell)
        current_row.append(
            case_inline_button(
                text,
                f"case:{round_id}:{position}",
                get_case_icon_id(db, result_type),
                selected_position == position,
            )
        )
        if len(current_row) >= side:
    ...
    try:
        if action in {"count", "cases", "cells", "ячейки", "кейсы"}:
            ...
            label_settings = {
                ...
                "nft": (SETTING_CASE_NFT_LABEL, SETTING_CASE_NFT_ICON_ID, "NFT"),
                "нфт": (SETTING_CASE_NFT_LABEL, SETTING_CASE_NFT_ICON_ID, "NFT"),
                "15": (
            ...
            db.set_setting(icon_key, icon_id)
        else:
            await send_html(update.message, game_settings_text(db))
            return
...
async def handle_case_choice(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if selected_cell is None:
        selected_cell = make_star_case_cell(position)
...
def selected_prize_label(
    ...
    if result_type == "stars25":
        return "✓ 25⭐"
    if result_type in {"ordinary", "deleted"}:
        title = str(gift["title"])[:10] if gift else "gift"
        return f"✓ {title}"
    return "✓ пусто"
