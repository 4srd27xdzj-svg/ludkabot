...
TEMPLATE_DEFAULTS = {
    ...
    "owner_panel": (
        ...
        "Фигурок 15/25: <b>{case_stars15_variant_count}/{case_stars25_variant_count}</b>"
    ),
    ...
    "game_settings": (
        ...
        "15 Stars: <b>{case_stars15_label}</b>\n"
        "25 Stars: <b>{case_stars25_label}</b>\n"
        "Цвета closed/15/25/NFT: "
        "<b>{case_closed_style}/{case_stars15_style}/{case_stars25_style}/{case_nft_style}</b>\n"
        ...
        "<code>/game label closed</code>, <code>15</code>, <code>25</code> или <code>nft</code>."
    ),
    "game_saved": (
        "Готово.\n\n"
        "<b>Настройки игры</b>\n\n"
    ...
    "case_variants": (
        ...
        "<code>/game variant add 15 🧸</code>\n\n"
        "Добавить premium emoji: отправьте эмодзи отдельным сообщением и ответьте:\n"
        "<code>/game variant add 15</code>\n\n"
        "Удалить по номеру: <code>/game variant remove 15 2</code>\n"
        "Очистить набор: <code>/game variant clear 15</code>"
    ),
...
def save_case_variants(
    ...
    setting_key = variant_setting_key(result_type)
    if setting_key is None:
        raise ValueError("Фигурки можно настраивать только для 15 или 25.")
    db.set_setting(setting_key, json.dumps(variants, ensure_ascii=False))


def case_variant_target(value: str) -> str:
    normalized = value.strip().lower()
    if normalized in {"15", "stars15", "15stars"}:
        return "stars15"
    if normalized in {"25", "stars25", "25stars"}:
        return "stars25"
    raise ValueError("Фигурки можно настраивать только для 15 или 25.")
...
def case_style_target(value: str) -> tuple[str, str]:
    ...
    if normalized not in targets:
        raise ValueError("Кнопки: closed, 15, 25, nft, selected.")
...
def make_star_case_cell(position: int, db: Database | None = None) -> dict[str, object]:
    stars = random.choice([15, 25])
...
async def build_case_board(
    ...
    board = [make_star_case_cell(position, db) for position in range(1, case_count + 1)]
    nft_gift = await choose_owner_gift(config, db, "nft")
    if nft_gift is None:
        return board

    denominator = get_nft_chance_denominator(db)
    expected_nft_cells = case_count / denominator
    nft_cells = int(expected_nft_cells)
    if random.random() < expected_nft_cells - nft_cells:
        nft_cells += 1
    nft_cells = max(0, min(case_count, nft_cells))

    if nft_cells == 0:
        return board

    nft_positions = set(random.sample(range(1, case_count + 1), nft_cells))
    for index, cell in enumerate(board):
        if int(cell["position"]) not in nft_positions:
            continue
        gift = nft_gift
        if nft_cells > 1:
            gift = await choose_owner_gift(config, db, "nft") or nft_gift
        board[index] = {
            "position": cell["position"],
            "result_type": "nft",
            "title": str(gift.get("title", "NFT")),
            "url": str(gift.get("url", "")),
            "stars": "",
            "owned_gift_id": str(gift.get("owned_gift_id", "")),
        }
    return board
...
def variants_settings_text(db: Database) -> str:
    ...
    text15 = (
        "\n".join(format_case_variant(variant, index) for index, variant in enumerate(variants15, 1))
        if variants15
        else empty_text
    )
    text25 = (
        "\n".join(format_case_variant(variant, index) for index, variant in enumerate(variants25, 1))
        if variants25
        else empty_text
    )
    return render_template_with_raw(
        db,
        "case_variants",
        {"variants15": text15, "variants25": text25},
        {"variants15", "variants25"},
    )
...
async def manage_game(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    try:
        if action in {"count", "cases", "cells", "ячейки", "кейсы"}:
            ...
            if len(context.args) < 3:
                raise ValueError("Формат: /game variant add 15 🧸.")
            ...
            if variant_action in {"add", "добавить"}:
                ...
                if len(context.args) < 4:
                    raise ValueError("Формат: /game variant remove 15 2.")
            ...
            if setting is None:
                raise ValueError("Доступно: closed, nft, 15, 25.")
...
async def handle_case_choice(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if selected_cell is None:
        selected_cell = make_star_case_cell(position, db)
...
async def choose_case_result(config: BotConfig, db: Database) -> tuple[str, dict[str, object] | None]:
    ...
    if random.choice([15, 25]) == 15:
        return "stars15", {
            "type": "stars",
            "title": "15⭐",
            "url": "",
            "owned_gift_id": "stars15",
            "stars": 15,
        }
