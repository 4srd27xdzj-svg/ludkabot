    ...
    try:
        if action in {"count", "cases", "cells", "ячейки", "кейсы"}:
            ...
            if len(context.args) < 3:
                raise ValueError("Формат: /game label closed 🎁 или /game label nft NFT.")
            label_target = context.args[1].lower()
            label_value = " ".join(context.args[2:]).strip()
            if not label_value:
                raise ValueError("Подпись не может быть пустой.")
            if label_target in {"closed", "close", "box", "case", "empty", "закрытая"}:
                db.set_setting(SETTING_CASE_CLOSED_LABEL, label_value)
            elif label_target in {"nft", "нфт"}:
                db.set_setting(SETTING_CASE_NFT_LABEL, label_value)
            elif label_target in {"15", "stars15", "15stars"}:
                db.set_setting(SETTING_CASE_STARS15_LABEL, label_value)
            elif label_target in {"25", "stars25", "25stars"}:
                db.set_setting(SETTING_CASE_STARS25_LABEL, label_value)
            else:
...
async def handle_case_choice(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    label = result_button_label(db, result_type, gift)
    try:
        await query.edit_message_reply_markup(
            reply_markup=case_keyboard(db, round_id, case_round["case_count"], position, label)
        )
